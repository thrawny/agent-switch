...
- Surface pivot (user, 2026-08-03): the picker becomes a **GTK layer-shell sidebar** living in agent-switch (`~/code/agent-switch`, Rust, gtk4 + layer-shell + niri IPC + daemon). The live prototype now defaults to an always-visible 480 px left dock; Mod+S toggles its exclusive keyboard command mode. The old exclusive-zone-0 popup remains an easy A/B option. **Workspace = named area**; thread→area lives in durable state because parked/cold threads cannot derive it. The sidebar carries create / summon / park / settle / archive / rename.
...
- [Surface content](issues/06-surface-content.md) — copy t3code sidebar v2: persistent left dock with Mod+S keyboard mode (optional popup), static creation order (activity never reorders; in-flight fades, brightness = needs-a-human), 3-line cards + slim Settled / Archived shelves; snooze deferred; attention refined to Approval > Input > Working > Failed > Unread > Idle with a colorblind-safe Monokai treatment; keyboard-first; global view by default with `g` narrowing to the focused area; Waybar shows highest-only literal Done > Working > open Idle; jump-to-next remains designed for Mod+Shift+S; area rename deferred.
