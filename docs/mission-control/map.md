...
Every design decision for "glue v1" mission control is resolved and captured in a blueprint-tier spec (`docs/mission-control/blueprint.md`) ready to hand to build sessions: registry schema, status producers, attach verb, surfaces (sidebar / waybar / jump bind), and lifecycle verbs.
...
- Surface pivot (user, 2026-08-03): the picker becomes a **GTK layer-shell sidebar** (exclusive zone left, waybar-style, not floating) living in agent-switch (`~/code/agent-switch`, Rust, already has gtk4 + gtk4-layer-shell + niri IPC + daemon; user offered takeover). Mod+S summons it per area. **Workspace = named area** (e.g. one work workspace consolidating that domain's repos); thread→area lives in the registry (parked threads have no windows to derive from). Layer surfaces are per-output — per-workspace sidebar is rendered content following focused workspace, not a compositor feature. Sidebar carries the verbs: create / summon / park / settle / archive / rename.
...
- [Surface content](issues/06-surface-content.md) — copy t3code sidebar v2: toggled-only (Mod+S, exclusive keyboard/zone), flat per-area inbox in static creation order (activity never reorders; in-flight fades, brightness = needs-a-human), 3-line cards + slim Settled (expanded, paged) / Archived (collapsed, tombstones) shelves; snooze deferred with schema seam; attention refined to Approval > Input > Working > Failed > Unread > Idle (colorblind remap: orange/indigo/sky/magenta, brightness+✓ for Done); keyboard-first (j/k, 1–9, Enter, s/p/a/r/n); non-area workspaces get global view; waybar = aggregate counts only; jump-to-next = Mod+Shift+S (tier then FIFO, empty → open sidebar); area rename deferred to 04.
- [Lifecycle verbs](issues/03-lifecycle-verbs.md) — three axes: visibility derived (summoned/parked — park is spatial, not lifecycle), lifecycle stored (live→settled→archived), attention derived+fused (Needs input > Unread > Working > Idle, one stored `last_read_at`); settle promises recoverability not warmth; archive = reclaim verb (manual-only v1, unarchive first); resurrection lazy via summon from registry manifest + harness resume; auto-settle daemon-written (idle + no attention + 36h), un-settle on summon or hand-raise; vocabulary in `CONTEXT.md`; zmx-vs-window substrate spun out to [08](issues/08-thread-runtime-substrate.md).
