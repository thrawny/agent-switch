...
- Surface pivot (user, 2026-08-03): the picker becomes a **GTK layer-shell sidebar** (exclusive zone left, waybar-style, not floating) living in agent-switch (`~/code/agent-switch`, Rust, already has gtk4 + gtk4-layer-shell + niri IPC + daemon; user offered takeover). Mod+S summons it per area. **Workspace = named area** (e.g. one "kanel" workspace consolidating ~/work/kanel repos); thread→area lives in the registry (parked threads have no windows to derive from). Layer surfaces are per-output — per-workspace sidebar is rendered content following focused workspace, not a compositor feature. Sidebar carries the verbs: create / summon / park / settle / archive / rename.
